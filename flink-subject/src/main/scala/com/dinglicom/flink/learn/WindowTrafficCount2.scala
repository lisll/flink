...
object WindowTrafficCount2 {
  def main(args: Array[String]) {
    ...
    val resultStream = originalStream.map(x => {
      ...
      var ultraffic = line(38).toLong // u32ultraffic
      var dltraffic = line(39).toLong // u32dltraffic      
      var totalTraffic = ultraffic+dltraffic
      new Tuple2[String, Array[Long]](msisdn, Array(ultraffic, dltraffic, totalTraffic))
    }).keyBy(0).timeWindow(Time.minutes(1), Time.seconds(30)).reduce((a,b)=>{
      var newNumbers: Array[Long] = new Array[Long](a.f1.length)
      for(i <- 0 until a.f1.length){
        newNumbers(i) = a.f1(i) + b.f1(i)
      }
      new Tuple2(a.f0,newNumbers)
    }).map(x=>{
      var builder = new StringBuilder()
      builder.append(x.f0).append(",")
      for (i <- 0 until x.f1.length) {
        builder.append(x.f1(i)).append(",");
      }
      
      builder.substring(0, builder.length - 1)      
    })
    
		val hadoopSink = new BucketingSink[String]("hdfs://nameservice1/tmp/test/flink_out8");
